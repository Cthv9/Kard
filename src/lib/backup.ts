...
export interface KardBackupV2 {
  ...
  data: string
  // Optional in legacy backups (assumed 200_000 when absent). Recorded so
  // we can raise the iteration count for new exports without breaking import
  // of files generated by older clients.
  iterations?: number
}
...
export async function importBackup(
  ...
  if (backup.version === 2) {
    ...
    // Older exports didn't record the iteration count; fall back to the
    // legacy value so they can still be imported.
    const iterations = b2.iterations ?? LEGACY_PBKDF2_ITERATIONS
